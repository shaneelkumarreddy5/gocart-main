...
const StoreInfo = ({store}) => {
    return (
        <div className="flex-1 space-y-2 text-sm">
            {store.logo ? (
                <img src={store.logo} alt={store.name} className="max-w-20 max-h-20 object-contain shadow rounded-full max-sm:mx-auto" />
            ) : (
                <div className="max-w-20 max-h-20 w-20 h-20 shadow rounded-full max-sm:mx-auto bg-slate-100" />
            )}
            ...
            <div className="flex items-center gap-2 text-sm ">
                {store.user?.image ? (
                    <img src={store.user.image} alt={store.user.name} className="w-9 h-9 rounded-full" />
                ) : (
                    <div className="w-9 h-9 rounded-full bg-slate-100" />
                )}
                <div>
                    <p className="text-slate-600 font-medium">{store.user?.name || 'Vendor User'}</p>
                    <p className="text-slate-400">{store.user?.email || 'N/A'}</p>
                </div>
            </div>
        </div>
    )
