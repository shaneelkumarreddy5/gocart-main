...
const Footer = () => {
    const noPrefetchPaths = new Set(['/create-store'])
        ...
        <footer className="mx-6 bg-white">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 border-b border-slate-500/30 text-slate-500">
                    <div>
                        <BrandLogo />
                        <p className="max-w-[410px] mt-6 text-sm">Welcome to Glonni, your cashback-first destination for smart shopping across trusted vendors. From smartphones and wearables to everyday essentials, discover transparent pricing and pending-to-available rewards in one place.</p>
                                ...
                                <ul className="space-y-2.5">
                                    {section.links.map((link, i) => (
                                        <li key={i} className="flex items-center gap-2">
                                            {link.icon && <link.icon />}
                                            <Link href={link.path} prefetch={!noPrefetchPaths.has(link.path)} className="hover:underline transition">{link.text}</Link>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                ...
                <p className="py-4 text-sm text-slate-500">
                    Copyright 2025 © Glonni All Rights Reserved.
                </p>
            </div>
        </footer>
    );
